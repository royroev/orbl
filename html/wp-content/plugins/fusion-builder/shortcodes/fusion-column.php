...
if ( ! class_exists( 'FusionSC_Column' ) ) {
	...
	class FusionSC_Column extends Fusion_Element {
		...
		public function render( $atts, $content = '' ) {
			...
			$this->args = FusionBuilder::set_shortcode_defaults( $defaults, $atts, 'fusion_builder_column' );
			extract( $this->args );
			...
			if ( $is_gradient_color ) {
				$background_image_style .= 'background-image: ' . Fusion_Builder_Element_Helper::get_gradient_string( $this->args, 'column' );
			}
			...
			// Box shadow.
			if ( 'yes' === $box_shadow ) {
				$box_shadow_styles = Fusion_Builder_Element_Helper::get_box_shadow_styles(
					[
			...
			if ( $lazy_load ) {
				...
				if ( $is_gradient_color ) {
					$lazy_bg .= 'data-bg-gradient="' . Fusion_Builder_Element_Helper::get_gradient_string( $this->args ) . '"';
				}
			}
			...
			if ( ( 'none' === $hover_type && empty( $link ) ) || ( empty( $hover_type ) && empty( $link ) ) ) {
				...
				'<div ' . ( ! empty( $id ) ? 'id="' . esc_attr( $id ) . '"' : '' ) . esc_attr( $animation['data'] ) . ' class="fusion-layout-column fusion_builder_column fusion_builder_column_' . $type . ' ' . esc_attr( $classes ) . ' ' . ( ! empty( $type ) ? esc_attr( $type ) : '' ) . '" ' . $style . '>
					<div class="' . $wrapper_classes . '" style="' . $wrapper_style . $wrapper_style_bg . $bg_color_fix . '" ' . $lazy_bg . ' ' . $lazy_dims . ' data-bg-url="' . $background_image . '">
						' . $inner_content
						. $additional_bg_image_div . '
			...
			// Add filter styles.
			$filter_style = Fusion_Builder_Element_Helper::get_filter_style_element( $this->args, '.fusion-builder-column-' . $this->column_counter );
			if ( '' !== $filter_style ) {
				$output .= $filter_style;
			}
