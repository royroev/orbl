			...
			exitBuilder: function( event ) {
				...
				if ( ! linkTag.parent().hasClass( 'exit-to-dashboard' ) ) {
					if ( linkTag.parent().hasClass( 'exit-to-back-end' ) ) {
						link = linkTag.data( 'admin-url' ) + '?post=' + FusionApp.getPost( 'post_id' ) + '&action=edit';
					} else {
			...
			iconPicker: function() {
				...
				_.each( icons, function( icon, key ) {
					_.each( icon[ 1 ], function( iconSubset ) {
						if ( -1 !== self.settings.status_fontawesome.indexOf( iconSubset ) ) {
							outputSets[ iconSubset ] += '<span class="icon_preview icon-' + key + '" title="' + key + ' - ' + iconSubsets[ iconSubset ] + '"><i class="' + icon[ 0 ] + ' ' + iconSubset + '" data-name="' + icon[ 0 ].substr( 3 ) + '"></i></span>';
						}
					} );
				} );
				...
				_.each( icons, function( iconSet, IconSetKey ) {
					...
					_.each( iconSet.icons, function( icon ) {
						...
						output += '<span class="icon_preview icon-' + icon + '" title="' + iconSet.css_prefix + icon + '"><i class="' + iconSet.css_prefix + icon + '" data-name="' + icon + '"></i></span>';
					} );
					output += '</div>';
				} );
...
FusionPageBuilder.options.fusionExport = {
	...
	getFusionMeta: function() {
		...
		jQuery.each( FusionApp.data.postMeta, function( key, value ) {

			if ( 0 === key.indexOf( 'pyre_' ) || 0 === key.indexOf( 'sbg_' ) ) {
				meta[ key ] = value;
			}
		} );
...
FusionPageBuilder.options.fusionOptionUpload = {
	removeImage: function( event ) {
		...
		$field.val( '{"id":"","url":"","width":"","height":"","thumbnail":""}' ).trigger( 'change' );
		...
		if ( $multiselect.length ) {
			...
			$multiselect.each( function() {
				var $self              = jQuery( this ),
					...
					$selectAddNew      = $self.parent().find( '.fusion-multiselect-addnew' ),
					$selectSave        = $self.parent().find( '.fusion-multiselect-save' ),
					$selectCancel      = $self.parent().find( '.fusion-multiselect-cancel' );
				...
				$selectAddNew.on( 'click', function() {
					jQuery( this ).parent().parent().find( '.fusion-form-multiple-select.fusion-select-inited' ).hide();
					jQuery( this ).parent().parent().find( '.fusion-multiselect-addnew' ).hide();
					jQuery( this ).parent().parent().find( '.fusion-multiselect-addnew-section' ).show();
					jQuery( this ).parent().parent().find( 'input.fusion-addnew-input' ).off( 'change keyup' );
				} );

				// Clicked on Cancel.
				$selectCancel.on( 'click', function() {
					jQuery( this ).parent().parent().find( '.fusion-multiselect-addnew-section' ).hide();
					jQuery( this ).parent().parent().find( '.fusion-form-multiple-select.fusion-select-inited' ).show();
					jQuery( this ).parent().parent().find( '.fusion-multiselect-addnew' ).show();
				} );
				...
				$selectSave.on( 'click', function() {
					var terms    = [],
						...
						$current = jQuery( this ),
						$options = jQuery( this ).parent().parent().find( '.fusion-select-options' ),
						values   = jQuery( this ).parent().find( 'input.fusion-addnew-input' ).val();
					...
					values            = values.split( ',' );
					ajaxData.taxonomy = $current.parent().find( 'input.fusion-addnew-input' ).data( 'id' );
					...
					jQuery.post( fusionAppConfig.ajaxurl, ajaxData, function( response ) {
						...
						if ( 'object' === typeof response ) {
							jQuery.each( response, function( term, term_id ) {
								$options.append( '<input type="checkbox" id="category-' + term_id + '" name="category[]" value="' + term_id + '" data-label="' + term + '" class="fusion-select-option fusion-multi-select-option">' );
								$options.append( '<label for="category-' + term_id + '" class="fusion-select-label">' + term + '</label>' );
							} );

							// Remove Loader.
							$current.closest( 'li.fusion-builder-option' ).removeClass( 'partial-refresh-active' );

							$current.parent().find( '.fusion-multiselect-cancel' ).trigger( 'click' );
							$current.parent().find( 'input.fusion-addnew-input' ).val( '' );
						}
					} );
				} );
...
FusionPageBuilder.options.fusionRepeaterField = {
	...
	initRepeater: function( $repeater ) {
		...
		case 'PO':
			...
			option   = this.options[ param ];
			fields   = option.fields;
			values   = FusionApp.data.postMeta[ param ];
